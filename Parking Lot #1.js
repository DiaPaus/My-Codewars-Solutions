...
class ParkingLot {
    constructor(size) {
        this.size = size;
        this.vehicles = new Array(this.size).fill('free');
    }
    park(vehicle) {
        if (vehicle instanceof Bike) {
            let idx = this.vehicles.indexOf('free');
            if (idx >= 0) {
        ...
        if (vehicle instanceof Car) {
            let idx = this.vehicles.findIndex((el, i, array) => el == 'free' && array[i + 1] == 'free')
            if (idx >= 0) {
                this.vehicles[idx] = vehicle; this.vehicles[idx + 1] = vehicle;
                return true
            }
            return false
        }
        if (vehicle instanceof Van) {
            let idx = this.vehicles.findIndex((el, i, array) => el == 'free' && array[i + 1] == 'free' && array[i + 2] == 'free')
            if (idx >= 0) {
                this.vehicles[idx] = vehicle; this.vehicles[idx + 1] = vehicle; this.vehicles[idx + 2] = vehicle
                return true
            }
            return false
        }
    ...
    retrieve(license) {
        let vehicle = this.vehicles.find(el => { return el.license == license });
        if (vehicle) {
            this.vehicles = this.vehicles.map(el => el == vehicle ? 'free' : el);
            return true
        }
        return false
    }
