...
class Potion {
    ...
    mix(potion) {
        let newColor = [];
        this.color.forEach((el, i) => { newColor.push(Math.ceil((el * this.volume + potion.color[i] * potion.volume) / (this.volume + potion.volume))) });
        return new Potion(newColor, this.volume + potion.volume)
    }
}
